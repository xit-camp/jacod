...
class TopologicalSort {
    ...
    public static <T> List<T> sort(DirectedGraph<T> graph, Set<T> excluded) {
        ...
        for (T node : reversedGraph) {
            explore(node, reversedGraph, result, visited, expanded, excluded);
        }
     ...
     */
    private static <T> void explore(T node, DirectedGraph<T> graph, List<T> result, Set<T> visited, Set<T> expanded, Set<T> excluded) {
        ...
        for (T predecessor : graph.edgesFrom(node)) {
            explore(predecessor, graph, result, visited, expanded, excluded);
        }

        if (!excluded.contains(node)) result.add(node);
        expanded.add(node);
    }
